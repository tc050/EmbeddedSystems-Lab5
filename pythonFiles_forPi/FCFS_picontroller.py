...
# Controller gains (students tune)
Kp = 0.15
Ki = 0.05
    ...
    while True:
        try:
            ...
            if len(parts) >= 6 and parts[0] == "DATA":
                q_len_arrival = len(ready_queue)
                ready_queue.append(Task(msg, addr, recv_wall, q_len_arrival))

                global max_queue_len
                max_queue_len = max(max_queue_len, len(ready_queue))

            # ---------- FCFS EXECUTION ----------
            while ready_queue:
                q_len_start = len(ready_queue)
